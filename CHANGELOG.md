...
### Added
- napari labeling bridge from the browse viewer. A "Label window…" button hands the **currently visible depth window** (full resolution, including any SVD destriping) to napari via `deeplogger/gui/labeler.py:launch_labeler`. Two label modes share one Labels layer: freehand **mask painting** (napari's brush) and a **parametric sinusoid** picker — a magicgui panel with geological-unit sliders (depth/dip/azimuth/aperture + per-log diameter) whose "Add pick" rasterizes the fracture's sinusoidal trace via `labels.py:get_label`. Labels save as a `[image, mask]` `.pt` bundle (the training-data format). Pure helpers `build_label_mask`/`save_label_bundle` are unit-tested; the napari shell is manual-verify. Windows above `MAX_LABEL_ROWS` (20k full-res rows) are refused with a "zoom in" prompt. Selection uses the display bounds rather than a separate ROI drag. Picks are in-memory only for now (no structured `Fracture` table persisted yet); labeler uses `get_label` geometry, which differs from `apply_label` (band thickness, corrections) — to be reconciled.
