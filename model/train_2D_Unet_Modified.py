...
class RandomAugmentation:
    def __init__(self, horizontal_flip_prob=0.5, vertical_flip_prob=0.5):
        self.horizontal_flip = RandomHorizontalFlip(p=horizontal_flip_prob)
        self.vertical_flip = RandomVerticalFlip(p=vertical_flip_prob)

    def __call__(self, image, indices):
        if random.random() < 0.5:
            image = self.horizontal_flip(image)
            indices = self.horizontal_flip(indices)
        if random.random() < 0.5:
            image = self.vertical_flip(image)
            indices = self.vertical_flip(indices)
        return image, indices

augmentation_transform = RandomAugmentation()


date_string = datetime.date.today().strftime("%m_%d")

data_directory = DATA_DIR + 'Bedretto_Output' + os.sep
config = {"ch_lr_every": 10, "data_dir": data_directory, "validate_every": 25, "lr": [0.1], "ch_lr": 0.5,
          "momentum": 0.9, "use_nesterov": True, "smooth": 1.0, "batch_size": 16, "batch_size_val": 8,
...
#I want to put together all the datasets in the file_IDs list
normal_dataset = Dataset(file_IDs)

selected_samples = []

# Iterate over the complete dataset
for image, indices in normal_dataset:
    # Check if the indices contain at least one non-zero value
    if torch.any(indices > 0):
        # If yes, append the sample to the selected samples list
        selected_samples.append((image, indices))

    # Assuming selected_samples is a list of tuples containing (image, indices)
    # Stack images and indices into tensors
selected_images_tensor = torch.stack([sample[0] for sample in selected_samples])
selected_indices_tensor = torch.stack([sample[1] for sample in selected_samples])

    # Create a TensorDataset using the selected images and indices
complete_dataset = data.TensorDataset(selected_images_tensor, selected_indices_tensor)
...
for epoch in range(max_epochs):
    ...
    for batch_idx, (image_in, index_mask) in enumerate(training_loader):
        ...
        image_in, index_mask = augmentation_transform(image_in, index_mask)

        image_in = image_in.to(device).float()
        index_mask = index_mask.to(device).long()

        optimizer.zero_grad()
        loss = loss_function(forward_model(image_in), index_mask.float())
        loss.backward()
        ...
        for batch_idx, (image_in, index_mask) in enumerate(validation_loader):
            ...
            if avg_acc > max_acc:
                ...
                torch.save(forward_model.state_dict(),
                           os.path.join(config["model_dir"] + config["model_name"] +'filtered' +'-epoch-' + str(epoch) + '.pt'))
