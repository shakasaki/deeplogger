...
- [fix-pending-verify] **GUI stuck on data load** — root cause found 2026-06-05: `launch_viewer` re-converted the whole LAS on every open (`to_zarr` uses zarr `mode="w"`), blocking the UI for minutes on multi-GB logs. Fixed: reuse `<dir>/.cache/<stem>.zarr` if present + wait cursor on first convert (`viewer.py`). The PyQt6 event-loop fixes (`367052c`) were already correct. **Verify on the remote box:** repeat opens should now be instant; if a *first* convert still hangs forever, bug is inside `read()`/`build_depth_pyramid`, isolate with `python -m deeplogger.gui.viewer /path/to/log.las`.
